			...
			if (castle.gold < castle.sendWaveCost() && !CastleKingdom.DEBUG) {
				...
				Util.getFriendsInRange(Castle.computeValue(castle), LEVEL_THRESHHOLD, function(friends:Array):void {
					formatFriends(friends, castle.x - Util.minX - padding * 2, Util.maxY - Util.minY - 50, 5, function(pages:Array):void {
						_rightMenu = new ScrollMenu(castle.x + castle.width, Util.minY, pages, closeMenus, Util.assets[Assets.ATTACK_FRIENDS_LEFT_TITLE], Util.assets[Assets.BUTTON_CANCEL], FlxG.WHITE, 
					...
					Database.getEnemyInfo(function(units:Array):void {
						...
						_leftMenu = new ScrollMenu(Util.minX, Util.minY, pages, closeMenus, Util.assets[Assets.ATTACK_FRIENDS_RIGHT_TITLE], Util.assets[Assets.ATTACK_FRIENDS_BUTTON], FlxG.WHITE, padding, 
							castle.x - Util.minX, Util.maxY - Util.minY, 3, moveUnit);
						add(_leftMenu);
						
						_middleMenu = new ScrollMenu(castle.x, Util.minY, page, closeMenusAndSend, Util.assets[Assets.ATTACK_FRIENDS_MIDDLE_TITLE], Util.assets[Assets.BUTTON_CANCEL], FlxG.WHITE, padding, castle.width, Util.maxY - Util.minY, 3);
						add(_middleMenu);
					});
				}, Castle.computeValue);
			}
